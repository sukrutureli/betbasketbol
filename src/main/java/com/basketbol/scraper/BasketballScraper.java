...
public class BasketballScraper {
    ...
    private String[] scrapeDetailTeams(String url, String fallbackName) {
        String home = "-";
        String away = "-";
        String title = "-";
        ...
        try {
            driver.get(url);
            PageWaitUtils.waitForPageLoad(driver, 12); // sende varsa kullan; yoksa aşağıdaki basit sürümü ekledim

            // 1) Ana selector: HeaderTeams
            // a[data-test-id='TeamLink'] span[data-test-id='HeaderTeams']
            List<WebElement> headerTeams = driver.findElements(
                By.cssSelector("a[data-test-id='TeamLink'] span[data-test-id='HeaderTeams']")
            );

            if (headerTeams.size() >= 2) {
                home = safeText(headerTeams.get(0));
                away = safeText(headerTeams.get(1));
            } else {
                // 2) Alternatif: data-test-id HomeTeam / AwayTeam başlık alanı varsa
                List<WebElement> altHome = driver.findElements(By.cssSelector("[data-test-id='HomeTeam'] span"));
                List<WebElement> altAway = driver.findElements(By.cssSelector("[data-test-id='AwayTeam'] span"));
                if (!altHome.isEmpty() && !altAway.isEmpty()) {
                    home = safeText(altHome.get(0));
                    away = safeText(altAway.get(0));
                }
            }

            // 3) Hâlâ bulunamadıysa: link metinlerinden türet
            if ("-".equals(home) || "-".equals(away)) {
                List<WebElement> teamLinks = driver.findElements(By.cssSelector("a[data-test-id='TeamLink'] span"));
                if (teamLinks.size() >= 2) {
                    home = "-".equals(home) ? safeText(teamLinks.get(0)) : home;
                    away = "-".equals(away) ? safeText(teamLinks.get(1)) : away;
                }
            }

            // 4) Son çare: fallbackName "Home - Away" şeklindeyse böl
            if (("-".equals(home) || "-".equals(away)) && fallbackName != null && !fallbackName.isBlank()) {
                String[] parts = fallbackName.split("\\s*-\\s*|\\s+vs\\s+|\\s+VS\\s+");
                if (parts.length >= 2) {
                    if ("-".equals(home)) home = parts[0].trim();
                    if ("-".equals(away)) away = parts[1].trim();
                }
            }
            ...
            System.out.println("scrapeDetailTeams hata: " + e.getMessage());
            // Fallback: yine de bir title üretelim
            if (fallbackName != null && !fallbackName.isBlank()) {
                String[] parts = fallbackName.split("\\s*-\\s*|\\s+vs\\s+|\\s+VS\\s+");
                if (parts.length >= 2) {
                    home = "-".equals(home) ? parts[0].trim() : home;
                    away = "-".equals(away) ? parts[1].trim() : away;
        ...
        return new String[] { home, away, title };
    }
